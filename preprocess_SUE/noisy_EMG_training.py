...
def Classify(clf, featuresAll, bestParam):
    if clf == 'svm':
        model = Classification.trainSVM(featuresAll, bestParam)
    elif clf == 'svm_rbf':
        model = Classification.trainSVM_RBF(featuresAll, bestParam)
    elif clf == 'extratrees':
        model = Classification.trainExtraTrees(featuresAll, bestParam)
    elif clf == 'randomforest':
        model = Classification.trainRandomForest(featuresAll, bestParam)
    elif clf == 'knn':
        model = Classification.trainKNN(featuresAll, bestParam)
    elif clf == 'gradientboosting':
        model = Classification.trainGradientBoosting(featuresAll, bestParam)
