...
class MLP_snn(nn.Module):
    ...
    def forward(self, x, factor1, factor2, factor3):
        ...
        # x = self.fc1(x) 
        x0_spike = create_spike_input_cuda(x, C_level*Q_level, theta/factor1)
...
class ModelTrainer:
    ...
    def orthogonality_loss(self, model, beta):
        ...
        for name, param in model.named_parameters():
            if 'fc2.weight' in name or 'fc3.weight' in name:  # 只关注包含'weight'的参数
            # if 'weight' in name:
    ...
    def train_model(self, train_loader):
        ...
        for epoch in range(self.num_epochs):
            ...
            for inputs, labels in train_loader:
                ...
                # print('l2', l2 * 1e-6)
                # print('self.orthogonality_loss(self.model,0)',self.orthogonality_loss(self.model,0) * 1e-7)
                # print(self.criterion(outputs, labels))
                loss = self.criterion(outputs, labels) + l2 * 1e-4 + 1e-3 * self.orthogonality_loss(self.model,0) + 1e-4 * F.huber_loss(outputs, labels, delta=0.8)
                loss.backward()
                self.optimizer.step()
    ...
    def evaluate_model(self, test_loader, epsilon, noise=noise):
        self.snn.eval()
        self.model.eval()
